...
class SettingsStore {
  ...
  save(next) {
    const allowed = [
      "provider", "endpoint", "model", "alwaysOnTop", "scale", "sound",
      "freeWalk", "interactiveZones", "spicyLines", "behaviorIntensity"
    ];
    for (const key of allowed) {
      if (Object.hasOwn(next, key)) this.data[key] = next[key];
    }
    this.data.scale = Math.min(1.15, Math.max(0.6, Number(this.data.scale) || 1));
    this.data.behaviorIntensity = Math.min(1.8, Math.max(0.5, Number(this.data.behaviorIntensity) || 1));
