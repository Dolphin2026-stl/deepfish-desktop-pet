...
将带透明通道的 PNG 替换为 `assets/character.png` 即可。建议人物完整居中、四周保留少量透明边距，长宽比接近 4:5。当前版本使用透明立绘叠加 CSS 表情、道具和动作层；后续可在不改变行为协议的情况下替换为 Live2D、Spine 或逐帧精灵图渲染器。
